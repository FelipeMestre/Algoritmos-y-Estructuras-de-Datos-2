...
public class UtilGrafos {
    ...
    public static void imprimirWarshall(boolean[][] matriz, Map<Comparable, TVertice> vertices, String titulo){
        if (vertices != null && matriz.length == vertices.keySet().size()) {
            ...
            for (int i = 0; i < matriz.length; i++) {
                ...
                for (int j = 0; j < matriz.length; j++) {
                    {
                        Boolean valorCasilla = matriz[i][j];
                        System.out.print(rellenar(valorCasilla.toString(), largo, ' '));
                    }
                }
                System.out.println();
                System.out.println(linea);
    ...
    public static <T extends IGrafoDirigido> T cargarGrafo(String nomArchVert, String nomArchAdy, 
        ...
        for (String verticeString : vertices) {
            if ((verticeString != null) && (! verticeString.trim().equals(""))) {
                verticeString = verticeString.split(",")[0].trim();
        ...
        for (String aristaString : aristas) {
            if ((aristaString != null) && (aristaString.trim() != "")) {
                ...
                aristasList.add(new TArista(datos[0].trim(), datos[1].trim(), Double.parseDouble(datos[2])));
            }
        }
        try {
