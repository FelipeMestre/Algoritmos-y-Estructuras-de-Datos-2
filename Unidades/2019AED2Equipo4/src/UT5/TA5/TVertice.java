...
public class TVertice<T> implements IVertice, IVerticeKevinBacon {
    ...
    public void numBaconEnProfundidad(Comparable actor) {
        this.setBacon(0);
    ...
    public int numBacon(Comparable actor) {
        ...
        this.setVisitado(true);
        LinkedList<TVertice> cola = new LinkedList<>();
        if (this.getEtiqueta().equals(actor)) {
            return numeroBacon = 0;
        }
        cola.add(this);
        while (!cola.isEmpty()) {
            TVertice x = cola.pop();
    ...
    public void numBaconEnAmplitud(Comparable actor) {
